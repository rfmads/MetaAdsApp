...
SELECT 
    ...
    p.id AS post_row_id,
    CASE 
        WHEN p.platform = 'facebook' THEN 'facebook_story'
        WHEN p.platform = 'instagram' THEN 'instagram_permalink'
    END AS link_type
FROM ads a
JOIN creative_ads c ON a.creative_id = c.creative_id
JOIN posts p ON c.effective_object_story_id = p.post_id
-- Avoid duplicates if you run this multiple times
WHERE NOT EXISTS (
    SELECT 1 FROM ad_posts b WHERE b.ad_id = a.ad_id
);
    """)
