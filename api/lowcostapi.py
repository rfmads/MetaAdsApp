...
def fetch_ads_agg(ad_account_id: str, days: int, page_id: Optional[str]) -> List[Dict[str, Any]]:
...
def high_cost_ads():
    ...
    try:
        rows = fetch_ads_agg(ad_account_id=ad_account_id, days=days, page_id=page_id)

        currency = get_currency(ad_account_id)
        delta = pick_delta(currency)
        avg_cpr = compute_avg_cpr(rows)
        threshold = (avg_cpr + delta) if avg_cpr else delta
        ...
        for r in rows:
            ...
            if cpr > threshold:
                data.append({
                    "ad": {"id": r.get("ad_id"), "name": r.get("ad_name")},
...
def low_cost_ads():
    ...
    try:
        ...
        threshold = (avg_cpr - delta) if avg_cpr else 0.0
        if threshold < 0:
            threshold = 0.0
