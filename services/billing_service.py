...
from integrations.meta_graph_client import MetaInvalidFieldError
...
CURRENCY_EXPONENT = {
    "JPY": 0, "KRW": 0, # Currencies with no decimals
    "KWD": 3, "BHD": 3, "JOD": 3, # Currencies with 3 decimals
}

def _normalize_money(raw_value: Any, currency: Optional[str]) -> Optional[Decimal]:
    """ Converts Meta's minor units to major units (e.g., cents to dollars) """
    if raw_value is None or raw_value == "":
        return None
    
    try:
        v = Decimal(str(raw_value))
        exp = CURRENCY_EXPONENT.get((currency or "").upper(), 2) # Default is 2 (cents)
        divisor = Decimal(10) ** Decimal(exp)
        
        return (v / divisor).quantize(Decimal("0.001")) if exp == 3 else (v / divisor).quantize(Decimal("0.01"))
    except:
        return None

def _get_last_activity_date_from_db(ad_account_id: int) -> Optional[str]:
    """ Pulls the most recent insight date across all levels """
    sql = """
        SELECT MAX(last_date) as last_activity_date
        FROM (
            SELECT MAX(date) as last_date FROM campaigns_daily_insights i 
            JOIN campaigns c ON c.campaign_id = i.campaign_id WHERE c.ad_account_id = %(id)s
            UNION ALL
            SELECT MAX(date) FROM adset_daily_insights i 
            JOIN adsets s ON s.adset_id = i.adset_id 
            JOIN campaigns c ON c.campaign_id = s.campaign_id WHERE c.ad_account_id = %(id)s
            UNION ALL
            SELECT MAX(date) FROM ad_daily_insights i 
            JOIN ads a ON a.ad_id = i.ad_id 
            JOIN adsets s ON s.adset_id = a.adset_id 
            JOIN campaigns c ON c.campaign_id = s.campaign_id WHERE c.ad_account_id = %(id)s
        ) as combined
    """
    res = query_dict(sql, {"id": ad_account_id})
    return str(res[0]["last_activity_date"]) if res and res[0]["last_activity_date"] else None

def sync_billing_for_account(client, ad_account_id: int, portfolio_code: str = "") -> Dict[str, Any]:
    act = f"act_{ad_account_id}"
    
    try:
        try:
            # Attempt with daily_spend_limit
            acc = client.get(act, params={"fields": FIELDS_WITH_DAILY})
        except MetaInvalidFieldError:
            # This now happens INSTANTLY without 3 retries or error logs
            logger.info(f"ℹ️ daily_spend_limit not supported for {act}, falling back.")
            acc = client.get(act, params={"fields": FIELDS_BASE})
        except Exception as e:
            raise e

        currency = acc.get("currency")
        
        # 2. Normalize Money Fields
        # acc.get("daily_spend_limit") will naturally return None if it wasn't fetched
        billing_data = {
            "ad_account_id": ad_account_id,
            "last_activity_date": _get_last_activity_date_from_db(ad_account_id),
            "amount_spent": _normalize_money(acc.get("amount_spent"), currency),
            "balance": _normalize_money(acc.get("balance"), currency),
            "spend_cap": _normalize_money(acc.get("spend_cap"), currency),
            "daily_spend_limit": _normalize_money(acc.get("daily_spend_limit"), currency),
            "account_status": acc.get("account_status"),
            "disable_reason": acc.get("disable_reason"),
        }

        # 3. Upsert into DB 
        # (Make sure your 'billing' table has the 'daily_spend_limit' column)
        sql = """
            INSERT INTO billing (
                ad_account_id, last_activity_date, amount_spent, 
                balance, spend_cap, daily_spend_limit, account_status, disable_reason, checked_at
            ) VALUES (
                %(ad_account_id)s, %(last_activity_date)s, %(amount_spent)s, 
                %(balance)s, %(spend_cap)s, %(daily_spend_limit)s, %(account_status)s, %(disable_reason)s, NOW()
            )
            ON DUPLICATE KEY UPDATE
                ...
                daily_spend_limit = VALUES(daily_spend_limit),
        ...
        record = {k: (float(v) if isinstance(v, Decimal) else v) for k, v in billing_data.items()}
        execute(sql, record)

        return {"ok": True, "ad_account_id": ad_account_id}

    except Exception as e:
        logger.error(f"❌ Billing failed {act}: {e}")
